...
public:
  ...
  virtual float estimate(const board &b) const
  {
    ...
    { // Iterate through all the isomorphics
...
public:
  ...
  state select_best_move(const board &b) const
  {
    ...
    for (state *move = after; move != after + 4; move++)
    {
      if (move->assign(b))
      {
        ...
        for (int empty_index : empty_indexes)
        {
          next.set(empty_index, 1);
          value += prob_2 * estimate(next);
          next.set(empty_index, 0);
          next.set(empty_index, 2);
          value += prob_4 * estimate(next);
          next.set(empty_index, 0);
        }

        move->set_value(move->reward() + value / empty_indexes.size());
   ...
   */
  void make_statistic(size_t n, const board &b, int score, std::ofstream &log_file, int unit = 1000)
  {
...
int main(int argc, const char *argv[])
{
  ...
  std::string log_path = std::string("../log/2048-") + std::string(timestamp) + std::string(".log");
  std::string weight_path = std::string("../weights/2048-") + std::string(timestamp) + std::string(".bin");

  bool is_train = true;
  if (argc > 1)
  {
    weight_path = std::string(argv[1]);
    info << "load weights from " << weight_path << std::endl;
    is_train = false;
  }

  info << "TDL2048-Demo @ " << timestamp << std::endl;
  info << "log at " << log_path << std::endl;
  info << "weights at " << weight_path << std::endl;
  learning tdl;
  ...
  tdl.add_feature(new pattern({5, 9, 10, 13, 14, 15}));
  tdl.add_feature(new pattern({4, 8, 9, 10, 12, 13}));
  tdl.add_feature(new pattern({0, 4, 5, 6, 8, 9}));
  tdl.add_feature(new pattern({1, 2, 3, 6, 9, 10}));
  ...
  tdl.add_feature(new pattern({1, 2, 3, 5, 9}));
  tdl.add_feature(new pattern({0, 1, 2, 4, 6}));
  tdl.add_feature(new pattern({1, 2, 6, 10, 11}));
  ...
  tdl.add_feature(new pattern({4, 5, 6}));
  tdl.add_feature(new pattern({1, 2, 5}));
  tdl.add_feature(new pattern({1, 4, 5}));
  tdl.add_feature(new pattern({0, 1, 4}));

  // restore the model from file
  info << "load model" << std::endl;
  tdl.load(weight_path);
  info << "model loaded" << std::endl;

  std::ofstream log_file;
  log_file.open(log_path.c_str(), std::ios::out);
  if (!log_file.is_open())
  {
  ...
  for (size_t n = 1; n <= total; n++)
  {
    ...
    {
      info << "episode " << n << " done"
           << "; " << float(n)/total * 100 << "%" << "\r" << std::flush;
      if (n == total){
        info << std::endl;
      }
    }
    tdl.update_episode(path, alpha);
    tdl.make_statistic(n, b, score, log_file);
  ...
  if (is_train)
  {
    info << "save model" << std::endl;
    tdl.save(weight_path);
    info << "model saved" << std::endl;
  }
  info << "done" << std::endl;
  log_file.close();
  info << "log closed" << std::endl;
